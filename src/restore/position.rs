...
pub fn restore_window_position(current_client: &HyprClient, saved_client: &HyprClient) -> Result<(), Box<dyn Error>> {
    // Move to workspace
    ...
    if saved_client.floating {
        if !current_client.floating {
            ipc::dispatch(&format!("togglefloating address:{}", current_client.address))?;
        }
        ipc::move_window_pixel(&current_client.address, saved_client.at[0], saved_client.at[1])?;
        ipc::resize_window_pixel(&current_client.address, saved_client.size[0], saved_client.size[1])?;
    } else {
